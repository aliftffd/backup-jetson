        ...
        try:
            while True:
                try:
                    ...
                    if tag_data:
                        if 'E2 00 20 23 12 05 EE AA 00 01 00 73' in tag_data or 'E2 00 20 23 12 05 EE AA 00 01 00 87' in tag_data:
                            tag_id = 'E2 00 20 23 12 05 EE AA 00 01 00 73'
                            name = 'Tag 1'
                            print(f"Detected Tag 1: {tag_id}, Name: {name}")

                            self.socketio.emit('rfid_data', {'name': name, 'tag_id': tag_id})
                            self.log_to_csv(tag_id, name, duty_cycle)
                            
                            # Start decreasing the speed until it reaches 0 or reads Tag 1 again
                            while duty_cycle > 0:
                                duty_cycle -= 0.05
                                duty_cycle = max(duty_cycle, 0)
                                print(f"Decreasing speed: setting duty cycle to {duty_cycle}")
                                self.motor_controller.set_motor_duty_cycle(duty_cycle)
                                time.sleep(0.1)

                                # Check if Tag 1 is read again
                                tag_data = self.send_rfid_cmd('BB 00 22 00 00 22 7E')
                                if 'E2 00 20 23 12 05 EE AA 00 01 00 73' in tag_data or 'E2 00 20 23 12 05 EE AA 00 01 00 87' in tag_data:
                                    print("Tag 1 read again, stopping further decrease.")
                                    break
